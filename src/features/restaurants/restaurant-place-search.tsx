...
export function RestaurantPlaceSearch({
  ...
  const handlePick = (prediction: Prediction) => {
  ...
  if (searchEnabled === false) {
    return (
      <div
        className={cn(
          "rounded-lg border border-border/80 bg-background/95 px-3 py-2 text-xs text-muted-foreground shadow-sm backdrop-blur-sm",
          className,
        )}
      >
        {keyMissingHint}
